...
def _feature_snapshot(
    ...
    return {
        ...
        "team_rating_last_5": _mean(team_history, 5),
        "team_rating_last_10": _mean(team_history, 10),
        "opponent_rating_last_5": _mean(opponent_history, 5),
        "opponent_rating_last_10": _mean(opponent_history, 10),
...
def build_historical_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.duplicated(["player_id", "match_id", "map"]).any():
    ...
    for column in HISTORICAL_FEATURES:
        work[column] = result[column].to_numpy()
